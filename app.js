...
app.use((req, res, next) => {
  ...
  if (req.user && req.user.profilePicture && req.user.profilePicture.purl) {
    ...
    let modifiedProfilePic = originalUrl.replace(
      "/upload",
      "/upload/q_auto,e_blur:50,w_250,h_250"
    );
  ...
  // }
  // Allow access to resetPassword links that contain tokens
  const isResetPasswordRoute = req.path.startsWith("/resetPassword");
  ...
  asyncwrap(async (req, res) => {
    try {
      ...
      const newBlog = new Blog({
        ...
        images: req.file
          ? [{ imgUrl: req.file.path, imgFilename: req.file.filename }]
          : [],
      });
...
app.get(
  ...
  isLoggedIn,
  isAdmin,
  asyncwrap(viewIndividualListing)
);
...
app.get(
  ...
  isLoggedIn,
  isAdmin,
  asyncwrap(viewListingReview)
);
app.delete(
  "/admin/listing/:id/reviews/:reviewId",
  isLoggedIn,
  isAdmin,
  asyncwrap(deleteListingReview)
);
...
app.post(
  "/admin/feedbacks/:id/toggleDisplay",
  isLoggedIn,
  isAdmin,
  asyncwrap(displayFeedback)
);
...
app
  ...
  .post(
    ...
    (req, res) => {
      req.flash("success", "Welcome back to Travia!");
      //admin login
      if (req.user.isAdmin) {
        req.flash("success", "Welcome back to Travia! You are an admin.");
...
app.route("/forgot-password").get(forgotPassword).post(passwordResetLink);
...
app
  ...
  .get(resetPasswordTokenGet)
  .patch(resetPasswordTokenPatch);
...
app.post(
  "/listing",
  upload.array("listing[image]", 4),
  isLoggedIn,
  asyncwrap(createpost)
);
...
app.post(
  "/listing/:id/like",
  isLoggedIn,
  asyncwrap(listingController.likeListing)
);
...
// app.post(
//   "/bookings/payment/confirm/:bookingId",
//   bookingController.confirmPayment
// );
// Confirm booking → render payment page
app.post("/bookings/my-bookings/:id", bookingController.confirmBooking);

// Confirm payment → process payment
app.post(
  "/bookings/payment/confirm/:id", // use :id as Listing ID
  bookingController.confirmPayment
);
// Route to show all bookings for the logged-in user
app.get("/bookings/my", isLoggedIn, bookingController.showMyBookings);
// Feedback
